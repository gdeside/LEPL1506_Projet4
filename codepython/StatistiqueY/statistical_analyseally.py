...
for name in names:
    file1 = open("statsally_%s" % name, "w")
    for p in positions:
        file1.write("######################## Position {} ###############################################\n".format(
            positionsdico[p]))
        for n in ntrials:
            file1.write("Stats AllY pour %s en position %s et les essais %d et %d\n" % (name, p, n, n + 1))
            ...
            ecart1 = pos1[1]
            ecart1 = ecart1 / np.linalg.norm(ecart1)
            ...
            ecart2 = pos2[1]
            ecart2 = ecart2 / np.linalg.norm(ecart2)
