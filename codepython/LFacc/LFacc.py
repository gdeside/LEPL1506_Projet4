...
for name in names:
    for p in positions:
        axs = plt.figure(figsize=(15, 10), constrained_layout=True).subplots(5, 1)
        axs = trim_axs(axs, 5)
        a = 0
        for n in range(1, ntrials + 1):
            ...
            axs[a].set_ylabel("Acc")
            axs[a].plot(time,accX,label="acceleration",color="blue")
            axs[a].legend()
            par.plot(time,LF,label="LF",color="red")
            par.set_ylabel("LF")
            par.legend()
            a+=1
        plt.suptitle("%s LF and acc in %s" % (name,p))
        plt.savefig("%s_%s_FLacc.png" % (name,p))
