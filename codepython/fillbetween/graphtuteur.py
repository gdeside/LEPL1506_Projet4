    ...
    for n in ntrials:
        for p in positions:
            ...
            for i in range(len(cycle_starts)):
                ...
                allx.append(xbis[0:nb_magique])
                ybis = pos[1][cycle_starts[i]:cycle_ends[i]]
                ally.append(ybis[0:nb_magique])
            ...
            axs[a].fill_between(index, xmean + xstd, xmean - xstd, alpha=0.2, label=p)
            axs[a].set_facecolor(colors[n-1])
            axs[a].legend()
            axs[a + 5].plot(index, ymean)
            axs[a + 5].fill_between(index, ymean + ystd, ymean - ystd, alpha=0.2, label=p)
            axs[a + 5].legend()
            axs[a+5].set_facecolor(colors[n-1])
        a += 1
    plt.suptitle("%s x et y"%name)
