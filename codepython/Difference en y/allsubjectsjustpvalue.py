...
for p, ax in zip(positions, tup):
    ...
    for name in names:
        ...
        if ax == ax1:
            ...
            ax.text(index[0] + 0.25, 0.085, 'mean:%s' % transformpvalue(p2), fontsize=12)
            ax.text(index[0] + 0.25, 0.065, 'std:%s' % transformpvalue(pvalbis), fontsize=12)
        else:
            ax.plot(indexgraph1, plotarray1, linestyle='dotted', color=sujetcolor[name])
            ax.scatter(indexscatter1, openarray, color=sujetcolor[name], alpha=0.5, s=20)
            ax.plot(indexgraph2, plotarray2, linestyle='dotted', color=sujetcolor[name])
            ax.scatter(indexscatter2, closearray, color=sujetcolor[name], alpha=0.5, s=20)
            ax.text(index[0] + 0.25, 0.145, 'mean:%s' % transformpvalue(p2), fontsize=12)
            ax.text(index[0] + 0.25, 0.13, 'std:%s' % transformpvalue(pvalbis), fontsize=12)
...
plt.savefig("errorbar_en_y_for_allpvalue.png")
