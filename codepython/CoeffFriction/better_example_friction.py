...
plt.plot(index[0][0], index[0][1], linestyle='', marker='*',color=color1,label='thumb pre')
plt.plot(x, values[1][0] * (x ** (values[1][1] - 1)),color=color2,label='index post')
plt.plot(index[1][2], index[1][3], linestyle='', marker='.',color=color2,label='index post')
plt.plot(x, values[1][2] * (x ** (values[1][3] - 1)),color=color2,linestyle='dashed',label='thumb post')
plt.plot(index[1][0], index[1][1], linestyle='', marker='*',color=color2,label='thumb post')
props=dict(boxstyle='round',facecolor='wheat',alpha=0.5)
text='\n'.join(("index pre : k=%.3f and n=%.3f"%(values[0][0],values[0][1]),"thumb pre : k=%.3f and n=%.3f"%(values[0][2],
                                                                                                             values[0][3]),"index post : k=%.3f and n=%.3f"%(values[1][0],values[1][1]),"thumb post : k=%.3f and n=%.3f"%(values[1][2],values[1][3])))
plt.text(5.5,2.0,text,bbox=props,fontsize=17)
