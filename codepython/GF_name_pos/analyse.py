...
for n in names:
    for pos in positions:
        ...
        fig = plt.figure(figsize=[15, 7])
        plt.title("figures pour %s : LF en fonction du temps en postion %s.png" % (n, pos))
        plt.plot(time, np.mean(gf_train, axis=0), label="GF_train")
        plt.plot(time, np.mean(gf_nobind1, axis=0), label="GF_nobind1")
        plt.plot(time, np.mean(gf_nobind2, axis=0), label="GF_nobind2")
        plt.plot(time, np.mean(gf3_bind1, axis=0), label="GF_bind1")
        plt.plot(time, np.mean(gf3_bind2, axis=0), label="GF_bind2")
        plt.legend()
        plt.xlabel("time[s]")
        plt.ylabel("LF[N]")
        fig.savefig("%s_LF_%s.png" % (n, pos))
