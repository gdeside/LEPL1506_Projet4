        ...
        for n in ntrials:
            ...
            if not path.exists(file_path):
                ...
                for i in range(len(cycle_starts)):
                    id = np.where((time1 > time1[cycle_starts[i]]) & (time1 < time1[cycle_ends[i]]))
                    if n == 2 or n == 3:
                        arrayopen.append(np.abs(time[np.nanargmax(GF[id])] - time[np.nanargmax(LF[id])]))
                    if n == 5 or n == 4:
                        arrayclose.append(np.abs(time[np.nanargmax(GF[id])] - time[np.nanargmax(LF[id])]))
