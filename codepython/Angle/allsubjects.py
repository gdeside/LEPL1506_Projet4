...
for p, ax in zip(positions, tup):
    for name in names:
        ...
        for n in ntrials:
            ...
            if not path.exists(file_path):
                ...
                box.append(np.nanmean(angle))
                box1.append(np.nanstd(angle))
                index.append(n)
    ...
    if p == 'UD':
        ax.set_xlabel('blocs(#)')
fig.suptitle("Variation de l'angle dans le plan frontral pour tous les sujets")
plt.savefig("errorbar_angle_all.png")
